...
const BEATS: Record<Gesture, Gesture> = { rock: "scissors", paper: "rock", scissors: "paper" };
const EMOJI: Record<Gesture, string> = { rock: "✊", paper: "🖐️", scissors: "✌️" };
const WIN_SCORE = 5;

function getAiMove(history: Gesture[]): Gesture {
  if (history.length < 5) return CLASSES[Math.floor(Math.random() * 3)];
  const recent = history.slice(-10);
  const counts = CLASSES.map((c) => ({ c, n: recent.filter((x) => x === c).length }));
...
export default function Game() {
  ...
  const [lastResult, setLastResult] = useState<"player" | "ai" | "draw" | null>(null);
  ...
  const detect = useCallback(() => {
    if (!landmarkerRef.current || !videoRef.current || !canvasRef.current || !ready) {
   ...
   const canvas = canvasRef.current;
    ...
    try {
      ...
      if (results.landmarks.length > 0) {
        ...
        const stable = gestureBuffer.current.length === 10 && new Set(gestureBuffer.current).size === 1
          ? gestureBuffer.current[0] as Gesture : null;
    ...
    <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center">
      ...
      <div className="w-full bg-gray-900 px-6 py-3 flex items-center justify-between">
        <div className="flex flex-col items-center">
          ...
          <div className="flex gap-1 mt-1">
            {Array.from({ length: WIN_SCORE }).map((_, i) => (
              <div key={i} className={`w-3 h-3 rounded-full ${i < playerScore ? "bg-green-400" : "bg-gray-700"}`} />
            ))}
          </div>
        </div>

        <div className="text-center">
          <h1 className="text-xl font-bold tracking-widest text-white">RPS vs AI</h1>
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex gap-1 mt-1">
            {Array.from({ length: WIN_SCORE }).map((_, i) => (
              <div key={i} className={`w-3 h-3 rounded-full ${i < aiScore ? "bg-red-400" : "bg-gray-700"}`} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="relative w-full max-w-2xl mt-4 rounded-xl overflow-hidden border border-gray-800">
        ...
        {state === "waiting" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40">
            ...
            <p className="text-gray-300 text-sm mb-6">First to {WIN_SCORE} wins the match</p>
        ...
        {state === "result" && lastPlayer && lastAi && lastResult && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/70">
            <p className={`text-4xl font-black mb-4 ${lastResult === "player" ? "text-green-400" : lastResult === "ai" ? "text-red-400" : "text-yellow-400"}`}>
              {lastResult === "player" ? "YOU WIN!" : lastResult === "ai" ? "AI WINS!" : "DRAW!"}
            </p>
        ...
        {state === "gameover" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80">
            <p className={`text-5xl font-black mb-2 ${gameWinner === "player" ? "text-green-400" : "text-red-400"}`}>
              {gameWinner === "player" ? "VICTORY! 🏆" : "DEFEAT..."}
            </p>
            <p className="text-gray-300 mb-2">{gameWinner === "player" ? "You beat the AI!" : "The AI got you!"}</p>
